...
def analyze_text_with_deepseek(text, output_file=None, prompt_prefix=""):
    ...
    if not prompt_prefix:
        prompt_prefix = """The following is text extracted from a PDF form uploaded by a company. 
Please provide a concise, well-structured summary that includes:

1. Document type and purpose (e.g., registration form, training enrollment, exam application)
2. Company information (company name, address, and contact details if available)
3. Employee participation details:
   - Number of employees attending
   - Training or exam information
   - Special requirements or accommodations
4. Important dates, deadlines, or scheduled events
5. Authorization details (signatories, declarations, approval sections)
6. Missing or incomplete information that should be highlighted
7. Next steps required based on the form content

Format your response as a structured business brief.
If certain parts of the text appear unclear or incomplete due to OCR limitations, indicate this in your summary.
Summary:"""
...
if __name__ == "__main__":
    ...
    parser.add_argument("--direct-output", "-d", action="store_true",
                        help="Output analysis directly to stdout without saving to file")
